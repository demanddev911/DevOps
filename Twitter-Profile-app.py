...
def ai_detailed_report_page():
    ...
    sections = [
        ...
        ("influence_reach", "التأثير والوصول", 18),
        ("content_strategy", "استراتيجية المحتوى والاتصال", 28),
        ("network_relations", "الشبكة والعلاقات", 38),
        ("reputation_sentiment", "السمعة وتحليل المشاعر", 48),
        ("behavioral_analysis", "التحليل السلوكي والاستراتيجي", 58),
        ("business_implications", "الدلالات التجارية", 68),
        ("strategic_recommendations", "التوصيات الاستراتيجية", 78),
        ("future_growth", "التوقعات والنمو المستقبلي", 88),
    ]
    ...
    for idx, (section_key, section_title, progress_val) in enumerate(sections):
        ...
        if section_key == "introduction":
            prompt = f"""أنت محلّل أعمال استراتيجي وخبير في ذكاء وسائل التواصل الاجتماعي. اكتب ملخصاً تنفيذياً احترافياً لحساب @{username}.
...
1. هوية المستخدم وأسلوبه الرقمي
2. لماذا يجب على الشركة الاهتمام بوجوده على تويتر
3. القيمة الاستراتيجية المحتملة
4. التوصية الأولية (تعاون / مراقبة / استفادة / تجنب)

اكتب بأسلوب تنفيذي واضح يركز على القيمة التجارية. لا تستخدم رموز أو علامات نجمية.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 5000)
            
        elif section_key == "influence_reach":
            ...
            prompt = f"""أنت محلل تأثير رقمي وخبير في تقييم المؤثرين. حلل التأثير والوصول لحساب @{username}.
...
المطلوب - قسم كامل (500-700 كلمة) يتضمن:
1. جودة ونمو المتابعين
2. انتظام التفاعل وتحليله
3. مستوى التأثير (محدود / متوسط / واسع / مؤثر عام)
4. الخصائص الديموغرافية أو الجغرافية للجمهور
5. التقييم العام للوصول والتأثير
6. القيمة المقدرة للحساب من منظور تسويقي

اكتب بأسلوب تحليلي يركز على القيمة التجارية. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 8000)
            
        elif section_key == "content_strategy":
            prompt = f"""أنت خبير استراتيجية محتوى رقمي. حلل استراتيجية المحتوى والاتصال لحساب @{username}.

التغريدات:
{evidence_text}

المطلوب - قسم كامل (600-800 كلمة) يتضمن:
1. المواضيع الأساسية التي يركز عليها المستخدم
2. نبرة الخطاب (مهنية، عاطفية، نقدية، تحفيزية)
3. معدل النشر وأوقات الذروة
4. أكثر أنواع المحتوى جاذبية (نصوص، صور، روابط)
5. استراتيجية التفاعل مع الجمهور
6. التوصيات لتحسين استراتيجية المحتوى

اكتب بأسلوب تحليلي يركز على الأداء والفرص التجارية. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 8000)
            
        elif section_key == "network_relations":
            prompt = f"""أنت خبير تحليل الشبكات الاجتماعية. حلل الشبكة والعلاقات لحساب @{username}.

التغريدات:
{evidence_text}

المطلوب - قسم كامل (600-800 كلمة) يتضمن:
1. الحسابات الأكثر تفاعلاً معه أو التي يذكرها باستمرار
2. المجموعات أو المجتمعات التي ينتمي إليها
3. الجهات أو المؤثرون الذين يساهمون في توسيع نطاقه
4. تحليل قوة الشبكة وقيمتها
5. الفرص التجارية من خلال الشبكة
6. المخاطر أو نقاط الضعف في الشبكة

اكتب بأسلوب تحليلي يركز على القيمة الاستراتيجية للشبكة. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "reputation_sentiment":
            if df_comments is None or df_comments.empty:
                comments_sample = "لا توجد بيانات تعليقات متاحة"
            else:
                comments_sample = "\n".join([f"- {c[:100]}" for c in df_comments['comment_text'].head(50).tolist()])
            
            prompt = f"""أنت خبير تحليل السمعة والمشاعر الرقمية. حلل السمعة وتحليل المشاعر لحساب @{username}.
...
1. النظرة العامة تجاه المستخدم (إيجابية / سلبية / حيادية)
2. المواضيع التي تُثير الجدل أو الدعم
3. درجة الموثوقية والمصداقية
4. وجود علامات على تفاعل آلي أو حسابات وهمية
5. تحليل المشاعر السائدة في التعليقات
6. التأثير على السمعة العامة

اكتب بأسلوب تحليلي يركز على المخاطر والفرص السمعية. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "behavioral_analysis":
            prompt = f"""أنت خبير تحليل سلوكي استراتيجي. حلل السلوك والاستراتيجية لحساب @{username}.

التغريدات:
{evidence_text}

المطلوب - قسم كامل (800-1000 كلمة) يتضمن:
1. ما الذي يجعل المستخدم مؤثرًا (خبرة، جاذبية، جدل، موقف اجتماعي)
2. كيف يوجّه النقاشات العامة أو يؤثر على الرأي العام
3. مدى توافق محتواه مع قيم العلامة التجارية أو الشركة
4. مؤشرات الولاء والثقة من جمهوره
5. أنماط السلوك الرقمي والتفاعل
6. القوة والضعف في الاستراتيجية الحالية

اكتب بأسلوب تحليلي عميق يركز على الدوافع والأنماط السلوكية. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "business_implications":
            prompt = f"""أنت مستشار أعمال استراتيجي. حلل الدلالات التجارية لحساب @{username}.

التغريدات والبيانات:
{evidence_text}

المطلوب - قسم كامل (800-1000 كلمة) يتضمن:
1. إمكانيات التعاون أو الشراكة [عالية / متوسطة / منخفضة] مع التبرير
2. مستوى المخاطر [سمعية / تجارية / قانونية]
3. التموضع السوقي [قائد رأي / متابع للاتجاه / محفّز نقاش]
4. الفرص التسويقية أو الإعلامية المقترحة
5. القيمة المقدرة للحساب كشريك تجاري
6. المخاطر المحتملة في التعامل معه

اكتب بأسلوب تنفيذي يركز على القرار التجاري. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "strategic_recommendations":
            all_previous_analysis = ""
            for prev_key, prev_title, _ in sections[:-2]:
                if prev_key in st.session_state.ai_report_cache:
                    all_previous_analysis += f"\n\n=== {prev_title} ===\n{st.session_state.ai_report_cache[prev_key][:800]}"
            
            prompt = f"""أنت مستشار استراتيجي تنفيذي. بناءً على جميع التحليلات السابقة، قدم توصيات استراتيجية واضحة لحساب @{username}.

التحليلات السابقة:
{all_previous_analysis}

المطلوب - قسم كامل (1000-1200 كلمة) يتضمن توصيات محددة مرتبة حسب الأولوية:

1. التوصية الأولى: [مثال – إنشاء تعاون تسويقي للاستفادة من مكانته]
   - الهدف
   - الخطوات المقترحة
   - التأثير المتوقع
   - الجدول الزمني

2. التوصية الثانية: [مثال – مراقبة المحتوى بسبب موضوعات حساسة]
   - الهدف
   - الخطوات المقترحة
   - التأثير المتوقع
   - الجدول الزمني

3. التوصية الثالثة: [مثال – تبنّي نمط النشر أو أوقات الذروة]
   - الهدف
   - الخطوات المقترحة
   - التأثير المتوقع
   - الجدول الزمني

4. التوصية الرابعة: [مثال – استهداف جمهوره في حملات العلامة التجارية]
   - الهدف
   - الخطوات المقترحة
   - التأثير المتوقع
   - الجدول الزمني

5. التوصية الخامسة والأخيرة
   - الهدف
   - الخطوات المقترحة
   - التأثير المتوقع
   - الجدول الزمني

اكتب بأسلوب تنفيذي عملي قابل للتنفيذ. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 12000)
            
        elif section_key == "future_growth":
            prompt = f"""أنت محلل استراتيجي متخصص في التنبؤ والنمو. حلل التوقعات والنمو المستقبلي لحساب @{username}.

البيانات والاتجاهات:
{evidence_text}

المطلوب - قسم كامل (700-900 كلمة) يتضمن:
1. اتجاهات التفاعل المستقبلية (نمو / ثبات / انخفاض)
2. فرص التوسع الموضوعي أو الجغرافي
3. المؤشرات المقترحة للمراقبة في الربع القادم
4. السيناريوهات المحتملة (متفائل / واقعي / متشائم)
5. العوامل التي قد تؤثر على النمو
6. التوصيات لتعظيم فرص النمو

اكتب بأسلوب تحليلي تنبؤي مبني على البيانات. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
...
def ai_summary_report_page():
    ...
    required_sections = ["introduction", "influence_reach", "content_strategy", "network_relations", 
                        "reputation_sentiment", "behavioral_analysis", "business_implications", 
                        "strategic_recommendations", "future_growth"]
    ...
    sections_list = [
        ...
        ("influence_reach", "التأثير والوصول"),
        ("content_strategy", "استراتيجية المحتوى والاتصال"),
        ("network_relations", "الشبكة والعلاقات"),
        ("reputation_sentiment", "السمعة وتحليل المشاعر"),
        ("behavioral_analysis", "التحليل السلوكي والاستراتيجي"),
        ("business_implications", "الدلالات التجارية"),
        ("strategic_recommendations", "التوصيات الاستراتيجية"),
        ("future_growth", "التوقعات والنمو المستقبلي"),
    ]
...
المطلوب - اكتب ملخصاً تنفيذياً شاملاً للإدارة العليا (1500-2000 كلمة) يتضمن:

**الملخص التنفيذي العام**
نظرة شاملة عن الحساب وقيمته الاستراتيجية

**التأثير والوصول**
تقييم القوة الرقمية والجمهور

**استراتيجية المحتوى**
تحليل المحتوى والأسلوب الرقمي

**الشبكة والعلاقات**
تقييم الشبكة والارتباطات المهمة

**السمعة والمشاعر**
تحليل السمعة والمخاطر المحتملة

**التحليل السلوكي**
فهم الدوافع والأنماط السلوكية

**الدلالات التجارية**
الفرص والمخاطر من منظور تجاري

**التوصيات الاستراتيجية**
إجراءات محددة مرتبة حسب الأولوية

**التوقعات المستقبلية**
النمو المتوقع والسيناريوهات المحتملة

**القرار النهائي**
توصية واضحة: تعاون / مراقبة / استفادة / تجنب

اكتب بأسلوب تنفيذي واضح موجه لصناع القرار. استخدم الأرقام والبيانات. لا تستخدم رموز.
الرد بالعربية الاحترافية."""
            ...
            if not st.session_state.data_loaded or 'extracted_data' not in st.session_state:
                ...
                with col2:
                    ...
                    sections_list = [
                        ...
                        ("influence_reach", "Influence & Reach"),
                        ("content_strategy", "Content Strategy & Communication"),
                        ("network_relations", "Network & Relations"),
                        ("reputation_sentiment", "Reputation & Sentiment Analysis"),
                        ("behavioral_analysis", "Behavioral & Strategic Analysis"),
                        ("business_implications", "Business Implications"),
                        ("strategic_recommendations", "Strategic Recommendations"),
                        ("future_growth", "Future Growth & Projections"),
                    ]
