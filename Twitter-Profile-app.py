...
def ai_detailed_report_page():
    ...
    for idx, (section_key, section_title, progress_val) in enumerate(sections):
        ...
        if section_key == "executive_summary":
            ...
            avg_engagement = int((total_likes + total_retweets + total_replies) / len(df_tweets)) if len(df_tweets) > 0 else 0
            
            comments_count = len(df_comments) if df_comments is not None and not df_comments.empty else 0
            
            prompt = f"""أنت محلل سمعة رقمية خبير. اكتب ملخص تنفيذي واضح وشامل حق حساب @{username}.

معلومات الحساب:
- اسم المستخدم: @{username}
- إجمالي المنشورات: {len(df_tweets):,}
- إجمالي التعليقات: {comments_count:,}
- إجمالي الإعجابات: {total_likes:,}
- إجمالي إعادة النشر: {total_retweets:,}
- متوسط التفاعل: {avg_engagement:,}

عينة من المنشورات والتعليقات:
{evidence_text[:8000]}

المطلوب - اكتب ملخص تنفيذي شامل (500-700 كلمة) يتضمن:

1. **نظرة عامة عن الحساب**: طبيعة النشاط، التخصص، الجمهور المستهدف
2. **أبرز النتائج**: أهم ما لاحظته في المنشورات والتعليقات
3. **الأداء العام**: مستوى التفاعل والوصول
4. **السمعة الحالية**: هل الحساب إيجابي، سلبي، أو محايد بشكل عام؟
5. **التوصيات السريعة**: أهم 3-5 نقاط يجب الانتباه لها

**مهم جداً - طريقة كتابة روابط الإثبات:**
"الحساب يحظى بتفاعل عالي في منشوراته عن الخدمات [الإثبات: https://twitter.com/username/status/123456789]، بينما المنشورات الأخرى تواجه انتقادات [الإثبات: https://twitter.com/username/status/987654321]."

**القواعد الإلزامية:**
- كل نقطة تحليلية لازم يكون معاها رابط إثبات
- الرابط: [الإثبات: رابط المنشور/التعليق الكامل]
- استخدم الروابط الفعلية من المنشورات والتعليقات
...
الرد لازم يكون بالعربية الفصحى مع لمسة إماراتية."""
            ...
            prompt = f"""أنت محلل سمعة رقمية خبير متخصص في تحليل الإيجابيات والسلبيات. حلل حساب @{username} بدقة.

المنشورات والتعليقات مع روابطها:
{evidence_text}

المطلوب - اكتب قسماً كاملاً (700-1000 كلمة) يتضمن:

**أولاً: الإيجابيات (Strengths)**
1. اذكر 5-10 نقاط إيجابية واضحة في الحساب
2. لكل نقطة، حط أمثلة محددة مع روابط إثبات
3. ركز على: جودة المحتوى، التفاعل الإيجابي، الشفافية، الخدمة، إلخ

**ثانياً: السلبيات (Weaknesses)**
1. اذكر 5-10 نقاط سلبية واضحة في الحساب
2. لكل نقطة، حط أمثلة محددة مع روابط إثبات
3. ركز على: المشاكل المتكررة، الشكاوى، التعليقات السلبية، نقاط الضعف

**ثالثاً: الخلاصة**
- أيش أكثر: الإيجابيات ولا السلبيات؟
- شو تأثير هالإيجابيات والسلبيات على السمعة؟

**مهم جداً - طريقة كتابة روابط الإثبات:**
"الإيجابيات:
1. سرعة الرد على استفسارات العملاء [الإثبات: https://twitter.com/username/status/123456789]

السلبيات:
1. تكرار الشكاوى من التأخير في التسليم [الإثبات: https://twitter.com/username/status/987654321]"

**القواعد الإلزامية:**
- كل إيجابية وسلبية لازم يكون معاها 2-3 روابط إثبات
- الرابط: [الإثبات: رابط المنشور/التعليق الكامل]
- استخدم الروابط الفعلية من المنشورات والتعليقات

اكتب بأسلوب موضوعي ومتوازن. ما تستخدم رموز أو نجوم.
الرد لازم يكون بالعربية الفصحى مع لمسة إماراتية."""
            content = generate_ai_section(mistral, section_key, prompt, 10000)
            
        elif section_key == "complaints_classification":
            # استخراج التعليقات إذا كانت موجودة
            comments_text = ""
            if df_comments is not None and not df_comments.empty:
                comments_sample = df_comments.head(100)
                comments_text = "\n\n".join([
                    f"التعليق: {row['comment_text'][:150]}\nالرابط: {row['comment_url']}"
                    for _, row in comments_sample.iterrows()
                ])
            
            prompt = f"""أنت محلل سمعة رقمية متخصص في تصنيف الشكاوى وتقييم تأثيرها. حلل الشكاوى والمشاكل في حساب @{username}.

المنشورات مع روابطها:
{evidence_text}

التعليقات (إن وجدت):
{comments_text if comments_text else "لا توجد تعليقات"}

المطلوب - اكتب قسماً كاملاً (800-1200 كلمة) يتضمن:

**أولاً: تصنيف الشكاوى**
صنف جميع الشكاوى والمشاكل الموجودة في المنشورات والتعليقات إلى فئات:
...
الرد لازم يكون بالعربية الفصحى مع لمسة إماراتية."""
            ...
            prompt = f"""أنت محلل استراتيجي خبير في فهم الرأي العام والدوافع النفسية. حلل الأسباب العميقة خلف رأي الجمهور حول @{username}.
...
التعليقات:
{comments_text[:4000] if comments_text else "لا توجد تعليقات"}
