...
def ai_detailed_report_page():
    ...
    sections = [
        ...
        ("what_people_say", "ماذا يقول الناس عني", 18),
        ("complaints_issues", "الناس بتشتكي من إيه", 28),
        ("why_they_say", "ليه الناس بتقول كده", 38),
        ("how_to_improve", "إزاي أقدر أصلّح الصورة", 48),
        ("strategic_analysis", "التحليل الاستراتيجي العام", 58),
        ("practical_solutions_short", "الحلول العملية - قصيرة المدى", 68),
        ("practical_solutions_medium", "الحلول العملية - متوسطة المدى", 78),
        ("practical_solutions_long", "الحلول العملية - طويلة المدى", 88),
        ("monitoring_metrics", "المؤشرات المقترحة للمتابعة", 95),
    ]
    ...
    for idx, (section_key, section_title, progress_val) in enumerate(sections):
        ...
        if section_key == "introduction":
            prompt = f"""أنت محلل سمعة واستراتيجي مختص في إدارة الانطباع العام. اكتب ملخصاً تنفيذياً عن سمعة حساب @{username}.

عينة من التغريدات:
{chr(10).join([f"- {t[:150]}" for t in sample_tweets[:200]])}

عينة من التعليقات (ماذا يقول الناس):
{chr(10).join([f"- {c[:100]}" for c in sample_comments_list[:100]]) if sample_comments_list else "لا توجد تعليقات متاحة"}

المطلوب - اكتب ملخصاً تنفيذياً (300-400 كلمة) يتضمن:
1. نظرة الجمهور العامة لصاحب الحساب
2. اتجاه الانطباع العام: إيجابي / سلبي / مختلط
3. الأسباب الرئيسية للمدح أو النقد
4. التوصية الأولية: ما الذي يجب التركيز عليه

اكتب بأسلوب واضح ومباشر. لا تستخدم رموز.
الرد بالعربية الواضحة مع لمسة ودية."""
            content = generate_ai_section(gemini, section_key, prompt, 5000)
            
        elif section_key == "what_people_say":
            prompt = f"""أنت محلل محتوى ماهر. حلل ماذا يقول الناس عن حساب @{username}.

التغريدات:
{evidence_text}

التعليقات والردود:
{chr(10).join([f"- {c[:150]}" for c in sample_comments_list[:150]]) if sample_comments_list else "لا توجد تعليقات كافية"}

المطلوب - قسم كامل (700-900 كلمة) يتضمن:
1. أكثر المواضيع التي يذكرها الناس عند الحديث عن الحساب
2. الكلمات والعبارات المتكررة في المدح أو النقد
3. الانطباع العام مع أمثلة حقيقية من الردود:
   - ما يقولونه الإيجابي
   - ما يقولونه السلبي
   - المواقف المحايدة
4. نوع اللغة المستخدمة (مدح، نقد حاد، نقد بناء، استهزاء)
5. من هم الناس اللي بيتكلموا (عملاء، متابعين، معارضين)

اكتب بأسلوب قريب من الناس وواضح. استخدم أمثلة حقيقية. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "complaints_issues":
            prompt = f"""أنت خبير في تحليل شكاوى العملاء وإدارة المشاكل. حلل الشكاوى والمشاكل حول حساب @{username}.

التغريدات والتعليقات:
{evidence_text}
{chr(10).join([f"- {c[:150]}" for c in sample_comments_list[:150]]) if sample_comments_list else "لا توجد تعليقات كافية"}

المطلوب - قسم كامل (800-1000 كلمة) يتضمن:

رتّب الشكاوى من الأكثر تكراراً للأقل:

**الشكوى رقم 1:**
- وصف المشكلة بالتفصيل
- كم مرة اتكررت (نسبة تقريبية)
- مدى خطورة هذه الشكوى على السمعة
- من الذي يشتكي (نوع الجمهور)
- أمثلة من التعليقات
...
**الشكاوى الأخرى الأقل شيوعاً:**
[قائمة مختصرة]

**تقييم عام:**
ما هي أخطر شكوى يجب معالجتها فوراً؟

اكتب بأسلوب تحليلي دقيق مع أمثلة. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "why_they_say":
            prompt = f"""أنت محلل نفسي وسلوكي متخصص. حلل لماذا يقول الناس ما يقولونه عن حساب @{username}.

التغريدات والتعليقات:
{evidence_text}
{chr(10).join([f"- {c[:100]}" for c in sample_comments_list[:100]]) if sample_comments_list else "لا توجد تعليقات كافية"}

المطلوب - قسم كامل (700-900 كلمة) يتضمن:

**الأسباب وراء المدح:**
- ما الذي يجعلهم يمدحون؟ (جودة محتوى، تفاعل، مصداقية، شخصية مميزة)
- العوامل العاطفية (يحسون بقرب، ثقة، إلهام)
- العوامل العملية (يستفيدون من المحتوى، يحصلون على قيمة)

**الأسباب وراء النقد:**
- ما الذي يجعلهم ينتقدون؟ (محتوى ممل، قلة تفاعل، عدم مصداقية، تكرار)
- العوامل العاطفية (إحباط، خيبة أمل، شعور بالإهمال)
- العوامل العملية (توقعات لم تتحقق، وعود لم تنفذ)

**تأثير أسلوب التواصل:**
- هل نبرة المنشورات (رسمية/ودية/جادة) تؤثر على رأي الناس؟
- هل طريقة الرد على التعليقات تخلق انطباع إيجابي أو سلبي؟

**الخلاصة:**
ما هو السبب الجذري الأساسي الذي يصنع الانطباع العام؟

اكتب بأسلوب تحليلي عميق وإنساني. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "how_to_improve":
            prompt = f"""أنت مستشار سمعة وإدارة علاقات عامة. قدم حلول عملية لتحسين سمعة حساب @{username}.

التحليلات السابقة:
- الناس يقولون: {st.session_state.ai_report_cache.get('what_people_say', 'غير متوفر')[:500]}
- الشكاوى الرئيسية: {st.session_state.ai_report_cache.get('complaints_issues', 'غير متوفر')[:500]}
- الأسباب: {st.session_state.ai_report_cache.get('why_they_say', 'غير متوفر')[:500]}

المطلوب - قسم كامل (800-1000 كلمة) يتضمن حلول محددة:

**كيف أخلي الناس تتكلم عني كويس:**

**1. تحسين أسلوب التواصل:**
- ✅ [إجراء محدد: مثلاً تخصيص 30 دقيقة يومياً للرد على التعليقات]
- ✅ [إجراء محدد: استخدام نبرة أكثر ودية ومباشرة]

**2. معالجة الشكاوى:**
- ✅ [إجراء محدد: نشر منشور يعترف بالمشاكل ويوضح الحلول]
- ✅ [إجراء محدد: إنشاء قناة سريعة للتواصل المباشر]

**3. بناء الثقة:**
- ✅ [إجراء محدد: مشاركة محتوى شفاف عن كواليس العمل]
- ✅ [إجراء محدد: نشر شهادات العملاء الراضين]

**4. زيادة التفاعل الإيجابي:**
- ✅ [إجراء محدد: إطلاق حملة شكر للمتابعين الداعمين]
- ✅ [إجراء محدد: طرح أسئلة واستفتاءات لزيادة المشاركة]

**5. المحتوى:**
- ✅ [إجراء محدد: تنويع نوع المحتوى بناءً على ما يحبه الجمهور]
- ✅ [إجراء محدد: التركيز على المحتوى التعليمي/الترفيهي/الإلهامي]

**6. التعاون:**
- ✅ [إجراء محدد: التعاون مع مؤثرين موثوقين لتعزيز المصداقية]

كل إجراء يجب أن يكون قابل للتنفيذ فوراً. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "strategic_analysis":
            prompt = f"""أنت محلل استراتيجي. حلل الأنماط العامة لحساب @{username}.
...
**أنماط المحتوى:**
1. نوع المحتوى الذي يحصل على تفاعل إيجابي:
   - ما هو؟
   - لماذا ينجح؟

2. نوع المنشورات التي تسبب جدل أو سلبية:
   - ما هي؟
   - لماذا تفشل؟

**سلوك الجمهور عبر الزمن:**
- هل التفاعل يتحسن أو يتدهور أو ثابت؟
- ما هي المحطات المهمة (تغريدات مثيرة للجدل، أحداث)

**التوافق مع الأهداف:**
- هل الصورة الحالية تخدم أهداف العلامة/الشخصية؟
- ما الذي يجب تعديله استراتيجياً؟

**أوقات النشر والتفاعل:**
- متى ينشر وكيف يؤثر ذلك على التفاعل؟

اكتب بأسلوب تحليلي واضح. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "practical_solutions_short":
            prompt = f"""أنت مستشار تنفيذي عملي. قدم حلول قصيرة المدى (خلال أسبوع) لحساب @{username}.

التحليلات السابقة:
{chr(10).join([st.session_state.ai_report_cache.get(k, '')[:300] for k in ['what_people_say', 'complaints_issues', 'why_they_say', 'how_to_improve']])}

المطلوب - قسم كامل (600-800 كلمة) يتضمن:

🔹 **الحلول القصيرة المدى (تنفيذ فوري - خلال أسبوع):**

**الإجراء 1:**
- ما هو: [وصف مختصر]
- كيف أنفذه: [خطوات واضحة]
- النتيجة المتوقعة: [ماذا سيحدث]
- الوقت المطلوب: [كم ساعة/يوم]
...
**أولوية التنفيذ:**
ما هو أهم إجراء يجب البدء فيه اليوم؟

كل حل يجب أن يكون عملي وبسيط وسريع التنفيذ. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "practical_solutions_medium":
            prompt = f"""أنت مستشار استراتيجي. قدم حلول متوسطة المدى (خلال شهر) لحساب @{username}.

التحليلات السابقة:
{chr(10).join([st.session_state.ai_report_cache.get(k, '')[:300] for k in ['strategic_analysis', 'how_to_improve']])}

المطلوب - قسم كامل (700-900 كلمة) يتضمن:

🔹 **الحلول المتوسطة المدى (خلال شهر):**

**المشروع 1: تحسين خطة المحتوى**
- الهدف: [ما تريد تحقيقه]
- الخطوات: [1، 2، 3...]
- الموارد المطلوبة: [وقت، فريق، أدوات]
- المدة: [كم أسبوع]
- المؤشرات: [كيف تقيس النجاح]

**المشروع 2: زيادة الشفافية والتواصل**
[نفس التفصيل]

**المشروع 3: إطلاق حملة تحسين السمعة**
[نفس التفصيل]

**المشروع 4: بناء علاقات مع المؤثرين**
[نفس التفصيل]
...
اكتب بأسلوب تنفيذي واضح. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
            
        elif section_key == "practical_solutions_long":
            prompt = f"""أنت استراتيجي طويل المدى. قدم خطة استراتيجية (3-6 أشهر) لحساب @{username}.

التحليلات الكاملة:
{chr(10).join([st.session_state.ai_report_cache.get(k, '')[:200] for k in sections[:-3]])}

المطلوب - قسم كامل (800-1000 كلمة) يتضمن:

🔹 **الحلول الطويلة المدى (استراتيجية 3-6 أشهر):**

**الاستراتيجية 1: بناء خطة تواصل طويلة الأمد**
- الرؤية: [ما تريد أن تكون عليه السمعة بعد 6 أشهر]
- الأهداف القابلة للقياس: [أهداف SMART]
- المبادرات الرئيسية: [3-5 مبادرات]
- الشراكات المطلوبة: [من تحتاج]

**الاستراتيجية 2: إطلاق برنامج مسؤولية اجتماعية (CSR)**
- الفكرة: [ما هو البرنامج]
- الأثر المتوقع على السمعة: [كيف سيحسن الصورة]
- التنفيذ: [خطوات عامة]

**الاستراتيجية 3: تطوير نظام إدارة السمعة**
- الأدوات: [ما تحتاجه]
- الفريق: [من يدير]
- البروتوكولات: [كيف تتعامل مع الأزمات]

**الاستراتيجية 4: بناء مجتمع داعم**
- برنامج السفراء: [كيف تحول المعجبين لسفراء]
- المكافآت والتقدير: [نظام حوافز]

**خارطة طريق الـ 6 أشهر:**
- الشهر 1-2: [أولويات]
- الشهر 3-4: [أولويات]
- الشهر 5-6: [أولويات]
...
اكتب بأسلوب استراتيجي شامل. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 12000)
            
        elif section_key == "monitoring_metrics":
            prompt = f"""أنت خبير في قياس الأداء والمؤشرات. حدد المؤشرات لمتابعة تحسن السمعة لحساب @{username}.

المطلوب - قسم كامل (600-800 كلمة) يتضمن:

📈 **المؤشرات المقترحة للمتابعة:**

**1. مؤشرات المشاعر (Sentiment):**
- معدل التفاعل الإيجابي مقابل السلبي
- نسبة الردود الإيجابية
- الكلمات الأكثر استخداماً (إيجابية/سلبية)
- كيف تقيسها: [الطريقة]
...
- نسبة الشكاوى المتكررة
- كيف تقيسها: [الطريقة]

**4. مؤشرات المحتوى:**
- أداء أنواع المحتوى المختلفة
- أفضل أوقات النشر
- الموضوعات الأكثر نجاحاً
- كيف تقيسها: [الطريقة]

**5. مؤشرات السمعة العامة:**
- Net Promoter Score (NPS) - لو ممكن
- نسبة التوصيات
- ذكر إيجابي من مؤثرين
- كيف تقيسها: [الطريقة]

**لوحة متابعة مقترحة (Dashboard):**
[تصور لأهم 5-7 مؤشرات يجب مراقبتها يومياً]
...
اكتب بأسلوب عملي وقابل للتطبيق. لا تستخدم رموز.
الرد بالعربية الواضحة."""
            content = generate_ai_section(gemini, section_key, prompt, 10000)
...
def ai_summary_report_page():
    ...
    required_sections = ["introduction", "what_people_say", "complaints_issues", "why_they_say",
                        "how_to_improve", "strategic_analysis", "practical_solutions_short",
                        "practical_solutions_medium", "practical_solutions_long", "monitoring_metrics"]
    ...
    previous_sections = {}
    sections_list = [
        ("introduction", "الملخّص التنفيذي"),
        ("what_people_say", "ماذا يقول الناس عني"),
        ("complaints_issues", "الناس بتشتكي من إيه"),
        ("why_they_say", "ليه الناس بتقول كده"),
        ("how_to_improve", "إزاي أقدر أصلّح الصورة"),
        ("strategic_analysis", "التحليل الاستراتيجي العام"),
        ("practical_solutions_short", "الحلول العملية - قصيرة المدى"),
        ("practical_solutions_medium", "الحلول العملية - متوسطة المدى"),
        ("practical_solutions_long", "الحلول العملية - طويلة المدى"),
        ("monitoring_metrics", "المؤشرات المقترحة للمتابعة"),
    ]
...
**الملخص العام**
نظرة الجمهور والانطباع العام: إيجابي / سلبي / مختلط

**ماذا يقول الناس**
أهم ما يذكره الناس عند الحديث عن الحساب (مدح ونقد)

**الشكاوى الرئيسية**
أهم 3 شكاوى يجب معالجتها

**لماذا يقولون ذلك**
الأسباب الجذرية وراء المدح أو النقد

**كيف نصلح الصورة**
أهم 5 إجراءات يمكن تنفيذها فوراً

**التحليل الاستراتيجي**
الأنماط والاتجاهات العامة

**الحلول القصيرة المدى**
ما يمكن فعله خلال أسبوع

**الحلول المتوسطة المدى**
خطة الشهر القادم

**الحلول الطويلة المدى**
الاستراتيجية للـ 6 أشهر القادمة

**المؤشرات والمتابعة**
كيف تقيس التحسن

**الخلاصة النهائية**
أهم 3 أولويات يجب البدء بها الآن

اكتب بأسلوب واضح ومباشر. استخدم أمثلة. لا تستخدم رموز.
الرد بالعربية الواضحة والودية."""
        
        summary_content = generate_ai_section(gemini, "summary_conclusion", prompt, 12000)
            ...
            if not st.session_state.data_loaded or 'extracted_data' not in st.session_state:
                ...
                with col2:
                    ...
                    sections_list = [
                        ...
                        ("what_people_say", "What People Say About Me"),
                        ("complaints_issues", "Main Complaints & Issues"),
                        ("why_they_say", "Why They Say That"),
                        ("how_to_improve", "How to Improve"),
                        ("strategic_analysis", "Strategic Analysis"),
                        ("practical_solutions_short", "Short-term Solutions"),
                        ("practical_solutions_medium", "Medium-term Solutions"),
                        ("practical_solutions_long", "Long-term Solutions"),
