"""
Enhanced Google Gemini 1.5 Flash API Rate Limiter and Timeout Handler
Version: 1.0
...
class TokenBucket:
    ...
    capacity: int = 15  # Maximum tokens (Gemini has higher limits)
    refill_rate: float = 1.0  # Tokens per second
    tokens: float = 15.0  # Current tokens
...
class EnhancedGeminiAnalyzer:
    """
    Enhanced Google Gemini API analyzer with comprehensive rate limiting and error handling
    """
    ...
    def __init__(
        ...
        max_tokens: int = 4000,
        rate_limit_per_key: int = 15,  # Requests per minute per key (Gemini has higher limits)
        timeout: int = 60
    ):
        ...
        logger.info(f"EnhancedGeminiAnalyzer initialized with {len(api_keys)} keys")
    ...
    def analyze(
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                logger.error(f"Failed to configure Gemini: {str(e)[:100]}")
                health.record_failure("config_error")
                continue
            ...
            try:
                ...
                if response and response.text:
                    ...
                    health.record_failure("empty_response")
                    continue
                ...
                if "quota" in error_str or "rate" in error_str or "429" in error_str:
                    logger.warning(f"Rate limit for key ending in ...{key[-8:]}")
                    ...
                    logger.error(f"Authentication error for key ending in ...{key[-8:]}")
                    health.record_failure("auth_error")
                    health.cooldown_until = time.time() + 3600  # 1 hour cooldown
                    continue

                elif "500" in error_str or "503" in error_str:
                    logger.warning(f"Server error for key ending in ...{key[-8:]}")
...
if __name__ == "__main__":
    ...
    analyzer = EnhancedGeminiAnalyzer(
        api_keys=api_keys,
        rate_limit_per_key=15  # 15 requests per minute per key
    )
